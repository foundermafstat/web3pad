...
(define-read-only (calculate-level (exp uint))
  (if (> exp u0)
    (if (> (/ exp exp-per-level) max-level)
      max-level
      (/ exp exp-per-level)
    )
    u1
  ...
  (let (
    ...
    (begin
      ...
      (if (is-some (get nft-token-id session))
        (add-nft-exp (unwrap! (get nft-token-id session) err-invalid-params) exp-gained canonical-score)
        true
      )
...
(define-private (add-nft-exp (token-id uint) (exp-gain uint) (score uint))
  (let (
      ...
      (let (
        ...
        (begin
          ...
          (map-set nft-stats token-id {
            ...
            total-score: (+ (get total-score nft-entry) score)
          })
  ...
  (let (
    ...
    (begin
      ...
      (if (is-some ft-contract)
        ;; FT transfer - simplified for deployment (requires trait import in production)
        (let ((ftc (unwrap! ft-contract err-invalid-params)))
          ;; In production, implement proper FT transfer with trait
          ;; For now, mark as claimed (actual transfer handled off-chain)
          (begin
        ...
        ;; STX transfer
        (match (stx-transfer? amount (as-contract tx-sender) player)
          success (begin
            ;; Mark as claimed
            (map-set pending-rewards session-id (merge reward {
              claimed: true,
  ...
  (let (
    ...
    (if (is-some session)
      (let (
        ...
        (status (get status s))
        (dispute-deadline (+ (unwrap-panic end-block) dispute-window-blocks))
      )
        (and 
