 ...
 * - With loading state: <ThemeLogo isLoading={true} />
 */
...
export function ThemeLogo({ 
	...
	// Determine the current color based on loading state
	// If color is explicitly set, use it; otherwise use loading/loaded colors
	const currentColor = color !== 'currentColor' ? color : (isLoading ? loadingColor : loadedColor);
	
	// Debug logging for header logo
	if (className.includes('object-contain')) {
		console.log('ThemeLogo (header) - isLoading:', isLoading, 'currentColor:', currentColor);
	}
	...
	return (
		<svg
			viewBox="0 0 517 211"
			style={combinedStyle}
			className={`w3p-logo ${isLoading ? 'loading' : ''} ${className}`}
			role="img"
			aria-label="W3P Logo"
		>
			<path 
