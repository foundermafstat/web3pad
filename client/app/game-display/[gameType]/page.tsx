...
const TestGyroGameScreen = dynamic(
	() => import('@/components/TestGyroGameScreen'),
	{
		ssr: false,
		loading: () => (
			<div className="min-h-screen bg-gray-900 flex items-center justify-center">
...
export default function GameDisplayPage({ params }: PageProps) {
	...
	useEffect(() => {
		...
		console.log('[GameDisplayPage] URL params:', {
			roomId: id,
	...
	useEffect(() => {
		if (typeof window !== 'undefined' && roomId) {
			const url = `${ENV_CONFIG.CLIENT_URL}/game/${resolvedParams.gameType}?mode=controller&roomId=${roomId}`;
			setControllerUrl(url);
			console.log('[GameDisplayPage] QR URL:', url);
		}
	...
	useEffect(() => {
		...
		if (
			...
			console.log(
				'[GameDisplayPage] Skipping PixiJS init for',
				resolvedParams.gameType
			);
			return;
		}

		if (!isInitialized || !roomId || appRef.current) {
			console.log('[GameDisplayPage] Skip init:', {
				isInitialized,
				roomId,
		...
		const init = async () => {
			console.log('[GameDisplayPage] Starting game initialization...');
			...
			if (!pixiContainer.current) {
				console.error('[GameDisplayPage] Container not found!');
				return;
			}
			...
			console.log(`[GameDisplayPage] Canvas size: ${screenWidth}x${screenHeight}`);
			...
			if (pixiContainer.current) {
				...
				console.log(
					'[GameDisplayPage] Canvas added, bounds:',
		...
		// Connect to socket
		console.log('[GameDisplayPage] Connecting to socket...');
		console.log('[GameDisplayPage] Server URL:', ENV_CONFIG.SERVER_URL);
		const socket = io(ENV_CONFIG.SERVER_URL, {
			...
			socket.on('connect', () => {
				console.log('[GameDisplayPage] Socket connected!');
			...
			socket.on('roomCreated', (data: any) => {
				console.log('[GameDisplayPage] Room created:', data);
			...
			socket.on('disconnect', () => {
				console.log('[GameDisplayPage] Disconnected');
				setConnectionStatus('disconnected');
			});

			socket.on('connect_error', (error: any) => {
				console.error('[GameDisplayPage] Connection error:', error);
				setConnectionStatus('disconnected');
			});
			...
			socket.on('playerConnected', (player: Player) => {
				console.log('[GameDisplayPage] Player connected:', player.name);
			});

			socket.on('playerDisconnected', (playerId: string) => {
				console.log('[GameDisplayPage] Player disconnected:', playerId);
				removePlayer(playerId);
			});

			socketRef.current = socket;
			console.log('[GameDisplayPage] Initialization complete!');
		};

		init();

		return () => {
			console.log('[GameDisplayPage] Cleanup');
	...
	}, [isInitialized, roomId, resolvedParams.gameType]);
	...
	return (
		<div className="fixed inset-0 w-full h-full bg-gray-900 flex flex-col overflow-hidden">
			{/* Header */}
			<div className="bg-gray-800/90 backdrop-blur-sm px-4 py-2 flex items-center justify-between border-b border-gray-700/50 flex-shrink-0 z-10 mt-20">
				<button
